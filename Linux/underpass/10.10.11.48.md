...
lets google mosh-server
looks like this starts a remote sessions where we can connect to it via ssh...
...
MOSH CONNECT 60001 Gs9Z+U9wID0/rRTG80LCWQ
...
[mosh-server detached, pid = 1942]
```

i can connect via the client software:
```
mosh-client 127.0.0.1 60001
MOSH_KEY environment variable not found.
```

ill set the MOSH_KEY env var to the key above and it still fails, lets try all in one line:

```
MOSH_KEY=71IDqKuGV8VpNxXD4bGAoQ mosh-client 127.0.0.1 60001
```
